...
function toProtoMessage(raw) {
  ...
  if (raw.conversation || raw.extendedTextMessage || raw.imageMessage || raw.videoMessage || raw.audioMessage || raw.documentMessage || raw.stickerMessage) {
    return raw;
  }
    ...
    try {
      ...
      this.sock = makeWASocket({
        ...
        getMessage: async (key) => {
          if (key?.id) {
            // 1. Check if it was a message sent by the bot
            ...
            // 2. Check if it was cached in antiDelete / messageStore
          ...
          // Return empty conversation proto so Baileys completes handshake and phone never hangs
          return { conversation: '' };
        }
      });
