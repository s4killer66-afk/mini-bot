...
class AntiDeleteManager {
  ...
  async handleRevoke(sock, event) {
    ...
    // ── 100% SILENT MODE: ALWAYS DELIVER TO OWNER'S PRIVATE INBOX ──
    const targetJid = safety.getOwnerJid(sock);
    ...
    const isSelfChat = chatJid === targetJid || safety.isOwner(chatJid);
    ...
    console.log(`[AntiDelete] Silently recovered deleted message from ${senderInfo.display} in ${chatName} -> forwarding to inbox (${targetJid})`);
    ...
    if (text) {
      ...
      console.log(`[AntiDelete] Sending recovered text to ${targetJid}...`);
      const sent = await safety.safeSend(sock, targetJid, {
        text: output
      });
    ...
    try {
      ...
      if (innerMsg.imageMessage) {
        ...
        console.log(`[AntiDelete] Sending recovered photo to ${targetJid}...`);
        const sent = await safety.safeSend(sock, targetJid, {
          image: buffer,
          caption: output
        });
      ...
      if (innerMsg.videoMessage) {
        ...
        console.log(`[AntiDelete] Sending recovered video to ${targetJid}...`);
        const sent = await safety.safeSend(sock, targetJid, {
          video: buffer,
          caption: output
        });
      ...
      if (innerMsg.stickerMessage) {
        ...
        const buffer = await getBuffer(stream);
        await safety.safeSend(sock, targetJid, {
          text: `🛡️ *[SILENT ANTI-DELETE]*\n📍 *Source:* ${chatName}\n👤 *${senderInfo.display} deleted a sticker at ${timeStr}:*`
        });
        const sent = await safety.safeSend(sock, targetJid, { sticker: buffer });
