...
class ContactStore {
  ...
  async getGroupMetadata(sock, groupJid) {
    ...
    try {
      const fetchPromise = sock.groupMetadata(groupJid);
      const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 1500));
      const meta = await Promise.race([fetchPromise, timeoutPromise]);
      if (meta) {
        this.indexGroupMetadata(groupJid, meta);
        return meta;
      }
    } catch (err) {
      // Fallback to stale cache if request fails or times out
