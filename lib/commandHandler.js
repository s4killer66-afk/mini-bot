...
class CommandHandler {
  ...
  async handleMessage(sock, msg) {
    ...
    if (mode === 'self' && !isOwner) {
      return false; // In self mode, completely ignore non-owners (zero report risk)
    }
    if (mode === 'groups' && !isGroup && !isOwner) {
      return false; // In groups mode, ignore random stranger direct messages
    }
    ...
    // If bot is turned OFF, only allow owner to run 'mini' command to wake it up
    if (!safety.isBotEnabled()) {
      if (command.name !== 'mini' || !isOwner) {
        return false;
      }
    }
