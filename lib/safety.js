...
class SafetyManager {
  constructor() {
    ...
    this.MAX_GLOBAL_SENDS_PER_MIN = 40;     // Max 40 bot messages total in 60s
    this.USER_COOLDOWN_MS = 1500;           // 1.5s between commands per user
    this.MAX_ANTIDELETE_PER_WINDOW = 5;     // Max 5 anti-delete reposts per chat in 15s
    this.ANTIDELETE_WINDOW_MS = 15000;      // 15 seconds window

    // Human typing simulation delay range (ms)
    this.MIN_TYPING_DELAY = 600;
    this.MAX_TYPING_DELAY = 1200;
  ...
  async safeSend(sock, rawJid, content, options = {}) {
    ...
    // Fast typing presence (non-blocking in background)
    try {
    ...
    // Quoting a message where fromMe is true or when sending to self-chat causes primary device decryption delays
    const cleanOptions = { ...options };
    if (cleanOptions.quoted) {
      if (cleanOptions.quoted.key?.fromMe || jid === this.getOwnerJid(sock)) {
