...
class SafetyManager {
  ...
  normalizeJid(sock, jid) {
    ...
    // Strip device specifiers (e.g. 923056499820:1@s.whatsapp.net -> 923056499820@s.whatsapp.net)
  ...
  async safeSend(sock, rawJid, content, options = {}) {
    ...
    // Normalize any @lid recipient JID to @s.whatsapp.net so phones decrypt without session mismatch
    const jid = this.normalizeJid(sock, rawJid);

    if (!this.canSend(jid)) {
      return null;
    }

    // Always strip quoted option in bot replies to eliminate Signal ratchet reconciliation stalls on mobile phones
    const cleanOptions = { ...options };
    if (cleanOptions.quoted) {
      delete cleanOptions.quoted;
    }

    // ── Global Anti-Tag Protection Shield ──
    // Strictly prevent mass-tagging or tagging group members to protect group members from unwanted pings and spam
    if (jid.endsWith('@g.us')) {
      if (cleanOptions.mentions) delete cleanOptions.mentions;
      if (content?.mentions) delete content.mentions;
      if (cleanOptions.contextInfo?.mentionedJid) delete cleanOptions.contextInfo.mentionedJid;
      if (content?.contextInfo?.mentionedJid) delete content.contextInfo.mentionedJid;
    }

    // Pre-generate unique message ID to prevent race conditions during retry reconciliation
    let messageId = cleanOptions.messageId;
    if (!messageId) {
      const randomHex = Math.random().toString(36).substring(2, 10).toUpperCase();
      messageId = `3EB0${Date.now().toString(36).toUpperCase()}${randomHex}`;
      cleanOptions.messageId = messageId;
    }

    // Pre-cache proto message payload in memory and disk BEFORE socket relay
    const protoPayload = typeof content?.text === 'string'
      ? { extendedTextMessage: { text: content.text } }
      : (content?.message || content);

    this.botSentIds.add(messageId);
    this.storeSentMessage(messageId, protoPayload);
    try {
    ...
    const actualId = sent?.key?.id || messageId;
    console.log(`[Safety] safeSend to ${jid}:`, actualId ? `SUCCESS (id: ${actualId})` : 'FAILED');

    // Update with actual sent key and proto if available
    if (sent?.key?.id) {
      this.botSentIds.add(sent.key.id);
      const msgPayload = sent.message || protoPayload;
    ...
    return sent || { key: { id: messageId, remoteJid: jid }, message: protoPayload };
  }
  ...
  isOwner(jid) {
    ...
    // 1. Direct match with last sender who toggled power switch
    if (this.lastPowerSender) {
      const cleanPower = this.lastPowerSender.split('@')[0].split(':')[0].replace(/[^0-9]/g, '');
      if (cleanPower && (cleanJid === cleanPower || cleanJid.slice(-9) === cleanPower.slice(-9))) {
        return true;
      }
    }

    // 2. Check LID to Phone mapping in memory
    try {
      const contactStore = require('./contactStore');
    ...
    // 3. Match against configured owner numbers (handles 0305... and 92305... formats)
    const owners = (config.ownerNumbers || []).map(n => n.replace(/[^0-9]/g, ''));
    if (config.pairingNumber) {
      const cleanPair = config.pairingNumber.replace(/[^0-9]/g, '');
      if (cleanPair && !owners.includes(cleanPair)) owners.push(cleanPair);
    }
    return owners.some(o => {
      if (!o) return false;
      if (cleanJid === o || cleanJid.endsWith(o) || o.endsWith(cleanJid)) return true;
      if (cleanJid.length >= 9 && o.length >= 9) {
        return cleanJid.slice(-9) === o.slice(-9);
      }
      return false;
    });
  }
